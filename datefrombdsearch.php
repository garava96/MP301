...
include 'connection.php'; 
		...
		{ 
			while ($row = mysqli_fetch_row($result))
			{
				...
				printf("<script>a=%s</script>",$row[0]);
				printf("</p><p id=\"%s\" style=\"cursor: pointer; color:red\" onclick=\"ViewOpen(this)\">",$row[0]);
				$query2 = mysqli_query($mysqli,"SELECT count(*) FROM `Reviews` WHERE `id_cafe` = '".$row[0]."'  ");
				$result2 = $query2;
				$row2 = mysqli_fetch_row($result2);
				printf("<b>Отзывы( %s )</b></p>",$row2[0]);				
				printf("</div>");
			}
			mysqli_free_result($result);
			}
			printf ("<script>alert('Введите значение')</script>");
		}
		else
		{
	
			$query = mysqli_query($mysqli,"SELECT count(*) FROM `Coffee` WHERE LOWER(`Name`) like LOWER('%".$search."%')") ;
			$row = mysqli_fetch_row($query);
			if ($row[0]==0 or $row[0]=='"' or $row[0]=="'")
			{
		...
		mysqli_free_result($result);
			...
			{
				$query = mysqli_query($mysqli,"SELECT  * FROM `Information`,`Coffee` WHERE LOWER(`Coffee`.`Name`) like LOWER('%".$search."%') AND `Information`.`ID`=`Coffee`.`ID`");
				if ($result = $query) 
			{ 
			while ($row = mysqli_fetch_row($result))
			{
